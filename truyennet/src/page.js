...
function execute(url) {
  ...
  var bid = parseBookIdFromDoc(doc);
  if (!bid) return Response.error("Không tìm thấy book id trên trang: " + url);
  var slug = parseSlug(url);
  var pageCount = parseListPageCountFromDoc(doc);
  var pages = [];
  for (var i = 1; i <= pageCount; i++) {
    pages.push(BASE_URL + "/get/listchap/" + bid + "?page=" + i + "#slug=" + encodeURIComponent(slug));
  }

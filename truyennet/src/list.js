...
function execute(url, page) {
  var target = page ? normalizeUrl(page) : normalizeUrl(url);
  var response = fetch(target, { headers: { "User-Agent": USER_AGENT } });
  if (!response.ok) return Response.error("Không tải được danh sách: " + target);

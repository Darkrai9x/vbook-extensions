...
function execute(key, page) {
  var url = page ? normalizeUrl(page) : BASE_URL + "/tim-kiem?s=" + encodeURIComponent(key || "");
  var response = fetch(url, { headers: { "User-Agent": USER_AGENT } });
  if (!response.ok) return Response.error("Không tìm kiếm được: " + key);
  var doc = response.html();
  return Response.success(parseNovelItems(doc), findNextPage(doc));
}
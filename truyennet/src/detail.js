...
function execute(url) {
  ...
  var response = fetch(url, { headers: { "User-Agent": USER_AGENT } });
  if (!response.ok) return Response.error("Không tải được trang truyện: " + url);

  var doc = response.html();
  var name = firstText(doc, ["h1[itemprop=name]", ".mRightCol h1", "h1"]);
  var cover = absUrl(firstAttr(doc, ["meta[property=og:image]", ".book-info-pic img", "img[itemprop=image]"], "content") || firstAttr(doc, [".book-info-pic img", "img[itemprop=image]"], "src"));
  var author = firstText(doc, ["a[itemprop=author]", ".book-info-text li a[href*=/tac-gia/]" ]);
  var description = doc.select("[itemprop=description]").html() || doc.select("#gioithieu").html() || "";
  description = description.replace(/<script[\s\S]*?<\/script>/ig, "").replace(/<style[\s\S]*?<\/style>/ig, "").trim();

  var genres = [];
  doc.select(".li--genres a, .book-info-text a[href*=/the-loai/]").forEach(function (e) {
    var title = cleanText(e.text());
    var input = absUrl(e.attr("href"));
    if (title && input) genres.push({ title: title, input: input, script: "list.js" });
  });

  var detail = [];
  doc.select(".book-info-text li").forEach(function (li) {
    var text = cleanText(li.text());
    if (text && text.indexOf("Tác giả") !== 0 && text.indexOf("Thể loại") !== 0) detail.push(text);
  });

  var status = cleanText(doc.select(".label-status").text()).toLowerCase();
  return Response.success({
    name: name || parseSlug(url),
    cover: cover,
    host: BASE_URL,
    author: author,
    description: description,
    detail: detail.join("\n"),
    ongoing: status.indexOf("full") < 0 && status.indexOf("hoàn") < 0,
    genres: genres
  });
}
...
function execute() {
  var response = fetch(BASE_URL, { headers: { "User-Agent": USER_AGENT } });
  if (!response.ok) return Response.error("Không tải được danh sách thể loại");
  var doc = response.html();
  var list = [];
  var seen = {};
  doc.select("a[href*=/the-loai/]").forEach(function (a) {
    var title = cleanText(a.text());
    var input = absUrl(a.attr("href"));
    if (!title || !input || seen[input]) return;
    seen[input] = true;
    list.push({ title: title, input: input, script: "list.js" });
  });
